...
public:
  ...
  array1(T *v0) : state(unallocated) {Dimension(std::numeric_limits<int>::max(),v0);}
  ...
  void Store(std::vector<T> &a) const{
    for(unsigned int i=0; i < size; i++) a[i]=v[i];
  }
...
//#ifdef __ArrayExtensions
  double abs2(T number) const {return number*number;} // SKS: Hacked this in because abs2 below was seemingly not declared or defined anywhere else; seems to work for now.
  double Abs2() const {
    __checkSize();
    double norm=0.0;
...
template<class T> using _array1 = typename array1<T>::opt;
...
public:
  ...
  void Reallocate(unsigned int nx0, unsigned int ny0, size_t align=0) {
    this->Deallocate();
...
public:
  ...
  void Reallocate(unsigned int nx0, unsigned int ny0, unsigned int nz0,
		  size_t align=0){
    this->Deallocate();
...
public:
  ...
  Array1(T *v0, int ox0=0) {
    Dimension(std::numeric_limits<int>::max(),v0,ox0);
  }
